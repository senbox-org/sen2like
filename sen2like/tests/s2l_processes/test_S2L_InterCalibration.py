...
class TestS2L_InterCalibration(TestCase):
    ...
    def test_S2B_band01(self):
        ...
        self.assertEqual(
            image.filepath, result_image.filepath, "Result image should be the same for S2B with baseline > 4"
        )
    ...
    def test_landsat(self):
        _product_path = os.path.join(config.get("base_url"), "Landsat8", "196/29/LC81960292017318MTI00")
        context = ProcessingContext(config, "31TFJ")
        product = Landsat8Product(_product_path, context)
        product.working_dir = os.path.join(config.get("wd"), product.name)
        image = S2L_ImageFile(os.path.join(_product_path, "LC81960292017318MTI00_B3.TIF"))
