...
def main():
    ...
    args_parser.add_argument(
        "utm_file",
        help=(
            """CSV file having path_row/utm table, formatted as path,row,utm, example :
              1,1,31
              1,2,30
              1,3,29
              1,4,28"""
    ...
    create_req = (
        "CREATE TABLE l8tiles (" "PATH_ROW VARCHAR(7), PATH VARCHAR(3), ROW VARCHAR(3), LL_WKT VARCHAR, UTM INTEGER); "
    )
    ...
    for _, row in df.iterrows():
        ...
        values.append((row["WRS_ID"], row["PATH"], row["ROW"], row["LL_WKT"], f"POLYGON(({coord}))", row["UTM"]))
