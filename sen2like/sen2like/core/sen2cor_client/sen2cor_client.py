...
class Sen2corClient:
    ...
    def run(self, product):
        ...
        try:
            if product.mtl.mission in self.gipp_template:
                gipp_path = self.write_gipp(product)
                cmd = [
    ...
    def write_gipp(self, product):
        gipp_path = os.path.join(
            S2L_config.config.get('wd'), 'sen2cor', product.name, f'sen2cor_gipp_{self.out_mgrs}.xml')
        template_file = os.path.join(os.path.dirname(__file__), self.gipp_template[product.mtl.mission])
        logger.debug(f'GIPP template : {template_file}')
        ...
        logger.debug(f'Pixel center : ({y}, {x})')
        ...
        logger.info(f'GIPP L2A : {gipp_path}')
        return gipp_path
