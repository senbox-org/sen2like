...
Architecture is chosen using PSD schemas attached from https://sentinel.esa.int/documents/247904/685211/Sentinel-2-Products-Specification-Document
"""
...
import sys
sys.path.append('../..')
from core.QI_MTD.generic_writer import write_pretty_format
...
def generate_S2_structure_XML(out_xml, product_name=None, tile_name=None, save_xml=False):
    ...
    Structure based on 3 xsd files which were extracted from https://sentinel.esa.int/documents/247904/685211/Sentinel-2-Products-Specification-Document
    ...
    root.append(Comment('Product name convention : https://sentinel.esa.int/web/sentinel/user-guides/sentinel-2-msi/naming-convention'))

    # Folders to create in the root folder, following  2_User_Product_Level-2A_Structure.xsd
    aux_data = SubElement(root, 'AUX_DATA', attrib={'type': 'folder'})
    datastrip = SubElement(root, 'DATASTRIP', attrib={'type': 'folder'})
    rep_info = SubElement(root, 'rep_info', attrib={'type': 'folder'})
...
def append_tile_structure(img_data:Element, qi_data:Element, aux_data:Element):
    ...
    bands = SubElement(img_data, 'All_11_S2_CHANNELS', attrib={'type': 'file'})
    aot = SubElement(img_data, 'Aerosol_Optical_Thickness_map', attrib={'type': 'file'})
    wvp = SubElement(img_data, 'Water_Vapor_Map', attrib={'type': 'file'})
    wvp = SubElement(img_data, 'TCI_channel', attrib={'type': 'file', 'optional': 'True'})
    native = SubElement(img_data, 'NATIVE', attrib={'type': 'folder'})

    # Files to append to QI_DATA
    olqc = SubElement(qi_data, 'OLQC_reports_XML_formatted', attrib={'type': 'file'})
    gml = SubElement(qi_data, 'GML_Quality_Mask_files', attrib={'type': 'file'})
    cloud_c = SubElement(qi_data, 'Cloud_Confidence', attrib={'type': 'file'})
    cloud_c.append(Comment('Raster mask values range from 0 for high confidence clear sky to 100 for high confidence cloudy. Unsigned Integer. JPEG2000. 8bit. available at 20m and 60m resolution.'))
    snow_ice_c = SubElement(qi_data, 'Snow_Ice_Confidence', attrib={'type': 'file'})
    snow_ice_c.append(Comment('Raster mask values range from 0 for high confidence NO snow/ice to 100 for high confidence snow/ice. Unsigned Integer. JPEG2000. 8bit. available at 20m and 60m resolution.'))
    ...
    ddv_pixels.append(Comment('Raster mask of Dark Dense Vegetation pixels used during AOT retrieval processing (optional)'))
...
def append_datastrip_structure(datastrip:Element):
    ...
    datastrip_folder.append(Comment('Naming convention from https://sentinel.esa.int/documents/247904/685211/Sentinel-2-Products-Specification-Document at p74, p438'))

    ds_xml = SubElement(datastrip_folder, 'DataStrip_Metadata_File', attrib={'type': 'file'})
    ...
    for child in children:
        if isinstance(child.tag, str):  # Removing <function Comment>
            ...
            if child.attrib['type'].lower() == 'folder':
                ...
                with open(child_save_path, 'w'): pass
...
if __name__ == '__main__':
    """
    Launch the main to generate the xml file (change outfile and folder_path)
    """

    xml_outfile = '/home/gcavaro/PycharmProjects/poleeo/HLS-project/sen2like/core/QI_MTD/xml_backbones/S2_folder_backbone.xml'
    tree = generate_S2_structure_XML(xml_outfile, save_xml=True)
    write_pretty_format(xml_outfile)

    # Uncomment below to test the folder creation
    # folder_path = '/home/gcavaro/PycharmProjects/dev_Guillaume/xml_tuto/S2_product_test'
    # create_architecture(folder_path, tree, product_name='NEW_PRODUCT_NAME', tile_name='NEW_GRANULE_TILENAME',create_empty_files=True)