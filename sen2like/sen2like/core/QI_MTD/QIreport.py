...
from core.QI_MTD.generic_writer import find_element_by_path, getParentObjectNode, get_elem_path, remove_namespace, \
    append_namespace_to_path, chg_elm_with_tag, \
    change_elm, copy_children, replace_namespace_recursively, create_child, \
    write_pretty_format, MtdWriter
...
class QiWriter(MtdWriter):
    ...
    def _manual_replaces(self, product):
        ...
        ns = self._remove_children('./Data_Block/report/checkList[parentID="L2H_SBAF"]/check/extraValues', tag='value', attrs= {'name': 'SBAF_'})
        sensor = product.sensor_name
        for key, item in sorted(metadata.qi.items()):
            if 'SBAF_' in key:
                create_child(self.root_out, './Data_Block/report/checkList[parentID="L2H_SBAF"]/check/extraValues', tag=ns+'value', text=str(item),
                             attribs={"name": key})
        ...
        url_aux = os.path.join(metadata.mtd.get('product_{}_name'.format(self.H_F)), 'GRANULE', metadata.mtd.get('granule_{}_name'.format(self.H_F)), 'AUX_DATA')
        change_elm(self.root_out, rpath='./Data_Block/report/checkList/item', attr_to_change='url', new_value=urls)
        change_elm(self.root_out, rpath='./Data_Block/report/checkList[parentID="L2AAUX"]/item', attr_to_change='url', new_value=url_aux)
        change_elm(self.root_out, rpath='./Data_Block/report/checkList/item', attr_to_change='name', new_value=metadata.mtd.get('granule_{}_name'.format(self.H_F)))
