...
from core.QI_MTD.generic_writer import MtdWriter, find_element_by_path, getParentObjectNode, get_elem_path, remove_namespace, \
    append_namespace_to_path, chg_elm_with_tag, \
    change_elm, copy_children, replace_namespace_recursively, create_child , copy_elements, search_db, \
    rm_elm_with_tag, find_element_by_path
...
class MTD_writer_S2(MtdWriter):
    ...
    def _manual_replaces(self, product):
        ...
        change_elm(self.root_out, rpath='./General_Info/Product_Info/PRODUCT_URI', new_value=metadata.mtd.get('product_{}_name'.format(self.H_F)))
        change_elm(self.root_out, rpath='./General_Info/Product_Info/PROCESSING_LEVEL', new_value='Level-2{}'.format(self.H_F))
        ...
        self._remove_children('./General_Info/Product_Info/Product_Organisation/Granule_List/Granule')
        ...
        if not config.getboolean('doSbaf'):
            ...
            pass
            # FIXME : get product image characteristics from origin sensor (S2 here), copying from another template for example (see commented lines above)
        copy_elements(['./General_Info/Product_Image_Characteristics/Reflectance_Conversion/U'], self.root_in, self.root_out)

        #  Geometric_info
        # ---------------
        tilecode = product.mtl.mgrs
        if tilecode.startswith('T'): tilecode = tilecode[1:]
        ...
        # Auxiliary_Data_Info
        # -------------------
        self._remove_children('./Auxiliary_Data_Info/GIPP_List')
...
class MTD_writer_LS8(MtdWriter):
    ...
    def _manual_replaces(self, product):
        ...
        change_elm(self.root_out, rpath='./General_Info/Product_Info/PRODUCT_URI', new_value=metadata.mtd.get('product_{}_name'.format(self.H_F)))
        change_elm(self.root_out, rpath='./General_Info/Product_Info/PROCESSING_LEVEL', new_value='Level-2{}'.format(self.H_F))
        change_elm(self.root_out, rpath='./General_Info/Product_Info/PRODUCT_TYPE', new_value='L8OLI2{}'.format(self.H_F))
        ...
        change_elm(self.root_out, rpath='./General_Info/Product_Info/Datatake/DATATAKE_SENSING_START', new_value=acqdate)
        change_elm(self.root_out, rpath='./General_Info/Product_Info/Datatake/SENSING_ORBIT_NUMBER', new_value=config.get('relative_orbit'))
        ...
        if not config.getboolean('doSbaf'):
            # FIXME : get product image characteristics from origin sensor (LS8 here), copying from another template fro example
            pass
        U = distance_variation_corr(product.acqdate)
        change_elm(self.root_out, rpath='./General_Info/Product_Image_Characteristics/Reflectance_Conversion/U', new_value=str(U))
        ...
        # Auxiliary_Data_Info
        # -------------------
        self._remove_children('./Auxiliary_Data_Info/GIPP_List', exceptions=['Input_Product_Info'])
        ...
        # Quality_Indicators_Info
        # -----------------------
        self._remove_children('./Quality_Indicators_Info', exceptions=['Input_Product_Info'])
        change_elm(self.root_out, './Quality_Indicators_Info/Input_Product_Info', new_value=product.mtl.landsat_scene_id)
...
class MTD_tile_writer_S2(MtdWriter):
    ...
    def _manual_replaces(self, product):
        ...
        self._remove_children('./Geometric_Info/Tile_Angles', tag='Viewing_Incidence_Angles_Grids')
        angles_path = os.path.join('GRANULE', metadata.mtd.get('granule_{}_name'.format(self.H_F)), 'QI_DATA', metadata.mtd.get('ang_filename'))
        ...
        copy_elements(['./Geometric_Info/Tile_Angles/Mean_Viewing_Incidence_Angle_List'], self.root_in, self.root_out, self.root_bb)
        ...
        self._remove_children('./Quality_Indicators_Info/Pixel_Level_QI', tag='MASK_FILENAME')

        for mask in metadata.mtd.get('masks_{}'.format(self.H_F)):
            create_child(self.root_out, './Quality_Indicators_Info/Pixel_Level_QI', tag=mask.get('tag'), text=mask.get('text'),
                     attribs=mask.get('attribs'))

        msk_text = find_element_by_path(self.root_in, './Quality_Indicators_Info/Pixel_Level_QI/MASK_FILENAME')[0].text
        ...
        for ql in metadata.mtd.get('quicklooks_{}'.format(self.H_F)):
            ...
            create_child(self.root_out, './Quality_Indicators_Info', tag="{}_FILENAME".format(os.path.splitext(ql_name)[0]), text=ql_path)
...
class MTD_tile_writer_LS8(MtdWriter):
    ...
    def _manual_replaces(self, product):
        ...
        change_elm(self.root_out, './General_Info/Archiving_Info/ARCHIVING_TIME', new_value=metadata.hardcoded_values.get('L8_archiving_time'))
        ...
        change_elm(self.root_out, './Geometric_Info/Tile_Angles/Mean_Viewing_Incidence_Angle_List/Mean_Viewing_Incidence_Angle/ZENITH_ANGLE', new_value=str(VZA))
        change_elm(self.root_out, './Geometric_Info/Tile_Angles/Mean_Viewing_Incidence_Angle_List/Mean_Viewing_Incidence_Angle/AZIMUTH_ANGLE', new_value=str(VAA))
