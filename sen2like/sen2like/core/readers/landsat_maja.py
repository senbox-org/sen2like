...
class LandsatMajaMTL(MajaReader):
    ...
    def __init__(self, product_path):
        ...
        self.product_name = os.path.basename(
            os.path.dirname(self.mtl_file_name)
        ...
        for image_node in image_list_node:
            ...
            if nature == "Surface_Reflectance":
                ...
                self.aerosol_band = os.path.join(
                    self.product_path, image_node.findtext(".//Image_File_List/IMAGE_FILE")
                )
        ...
        for cpt, spectral_node in enumerate(spectral_nodes):
            ...
            self.rad_radio_coefficient_dic[str(cpt)] = {"Band_id": band_id, "Gain": gain, "Offset": offset}
            ...
            self.radio_coefficient_dic[str(cpt)] = {"Band_id": band_id, "Gain": gain, "Offset": offset}
    ...
    def can_read(product_name):
        return os.path.basename(product_name).startswith("LANDSAT8") or os.path.basename(product_name).startswith(
            "LANDSAT9"
        )
