...
class LandsatMTL(BaseReader):
    ...
    def __init__(self, product_path):
        ...
        try:
            ...
            self.product_name = os.path.basename(
                os.path.dirname(mtl_file_name)
            )  # PRODUCT_NAME # VDE : linux compatible
            ...
            self.scene_center_time = "".join([self.scene_center_time.split(".")[0], ".", second + "Z"])
            ...
            for cpt, k in enumerate(result):
                ...
                radio_coefficient_dic[str(cpt)] = {"Band_id": band_id_st, "Gain": v, "Offset": "0"}
            ...
            tif_files = [
                filename for filename in os.listdir(product_path) if re.search(r"\.tif$", filename, re.IGNORECASE)
            ]
    ...
    def _handle_l2a_quality(self, l2a_qi_report_path: str):
        ...
        root = tree.getroot()
        element = tree.find(
            "./Data_Block/report/checkList/check/extraValues/value[@name='CLOUDY_PIXEL_PERCENTAGE']", root.nsmap
        )
    ...
    def _get_band(self, regex):
        image_list = [
            filename for filename in os.listdir(self.product_path) if re.search(regex, filename, re.IGNORECASE)
        ]
    ...
    def can_read(product_name):
        ...
        return (
            ...
            or (name.startswith("L2F") and ("_LS8_" in name or "_LS9_" in name))
        )
