...
class ECMWF_Product:
    ...
    def __init__(self, cams_config, observation_datetime=None):
        ...
        for parameter in self.mandatory_attributes:
            ...
            if getattr(self, parameter) is None:
                ...
                log.info(
                    "Note that the CAMS monthly database for the Year 2020 is available : " "http://185.178.85.51/CAMS/"
                )
                self.is_valid = False
                return
    ...
    def set_monthly_array(self, nc_file, observation_date):
        ...
        hour1900 = (acqdate - datetime.datetime(1900, 1, 1)).total_seconds() / 60.0 / 60.0
        ...
        v1 = datetime.datetime(1900, 1, 1) + timedelta(
            hours=int(nctimes[index1])
        )  # VDE : failure for me if hours is type np.int32
        index2 = val[1][1]
    ...
    def read_cams_hourly(self, parameter):
        ...
        h1900 = (self.observation_datetime - datetime.datetime(1900, 1, 1)).total_seconds() / 60.0 / 60.0
    ...
    def read_cams_climatology(self, doy, parameter):
        ...
        file_name = os.path.join(
            self.cams_climatology_directory, f"CAMS_Climatology_2010-2019_{parameter}_DOY_{doy:03d}.tif"
        )
    ...
    def _find_cams_daily_nc(self, hour1900, parameter):
        ...
        nc_name = (
            f"CAMS_archive_aod550_tcwv_msl_gtco3_analysis_0H_6H_12H_18H_{date.year:4}-{date.month:02}-{date.day:02}.nc"
        )
