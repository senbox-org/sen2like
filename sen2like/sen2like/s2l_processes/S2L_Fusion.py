...
class S2L_Fusion(S2L_Process):
    ...
    def preprocess(self, product: S2L_Product):
        ...
        if not product.fusionable:
            log.info("Not enought reference products for fusion, fusion process will be skipped")
            return

        self._set_fusion_method()
    ...
    def _get_qa_band(self, shape):
        ...
        log.info("Get reference products BQA mask")
        ...
        for i in range(1, number_of_dates + 1, 1):
            ref_product = self.reference_products[i - 1]
            ...
            if msk.shape != shape:
                ...
                msk = skit_resize(msk.clip(min=-1.0, max=1.0), shape, order=0, preserve_range=True).astype(np.uint8)

            # 2024/12/12: see https://github.com/numpy/numpy/issues/7225
            # force np.uint8 to avoid unsafe cast
            msk_qa += msk * np.power(2, i, dtype=np.uint8)
    ...
    def _predict(self, product: S2L_Product, band_s2, qa_mask, output_shape):
        ...
        for mode in output_images.keys():
            ...
            # image2 (newest date)
            array2 = self.get_harmonized_product(ref_product_2, band_s2, output_shape, plus)
