...
class S2L_PackagerL2F(S2L_Process):
    ...
    def preprocess(self, product):
        ...
        metadata.mtd['product_creation_date'] = dt.datetime.now() if metadata.mtd.get('product_creation_date',
                                                                                      "NONE") is "NONE" \
            else metadata.mtd.get('product_creation_date')
    ...
    def postprocess(self, pd):
        ...
        Qi_Writer = QiWriter(bb_QI_path, outfile=out_QI_path, init_QI_path=in_QI_path, H_F='F')
